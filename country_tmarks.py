...
def main():
    connect("combosquatting_country_tmark", host="127.0.0.1")
    ...
    for tmark in gen_model.tmarks:
        for country in gen_model.countries_and_tlds.keys():
            gen_model.visit_domain(tmark, country + tmark + gen_model.countries_and_tlds[country], False)
            gen_model.visit_domain(tmark, country + '-' + tmark + gen_model.countries_and_tlds[country], False)
            for main_tld in gen_model.main_tlds:
                gen_model.visit_domain(tmark, country + tmark + main_tld, False)
                gen_model.visit_domain(tmark, country + '-' + tmark + main_tld, False)
        ...
        for country in gen_model.us_names:
            gen_model.visit_domain(tmark, country + tmark + '.us', False)
            gen_model.visit_domain(tmark, country + '-' + tmark + '.us', False)
            for main_tld in gen_model.main_tlds:
                gen_model.visit_domain(tmark, country + tmark + main_tld, False)
                gen_model.visit_domain(tmark, country + '-' + tmark + main_tld, False)
        ...
        for country in ["britain", "uk", "england"]:            
           gen_model.visit_domain(tmark, country + tmark + ".co.uk", False)
           is_last = country == "england" and tmark == gen_model.tmarks[-1]
           gen_model.visit_domain(tmark, country + '-' + tmark + ".co.uk", is_last)
